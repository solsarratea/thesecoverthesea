...
export function Controls(props: ControlsProps) {
  ...
  const setPivotControls = useLayersManager(
    (state: any) => state.setPivotControls
  );
  ...
  return (
    ...
    >
      ...
      <section className="selector">
        ...
        <hr />
        <button
          className={!instructions ? "" : "selected"}
          onClick={() => {
        ...
        {welcome ? (
          <>
            <section className="text scroll">
              <p>
                This is a 3D chromatic layer editor from the sketches of Georg
                Forsters.
              </p>

              <p>
                It is part of the project{" "}
                <a href="https://www.hackdash.org/projects/634ab7a16d202d739f69ce81">
                  THESECOVER THE SEA
                </a>{" "}
                developed during the hackathon{" "}
                <a href="https://creating-new-dimensions.org/">
            ...
            <section className="text scroll">
              <p>
                Design a postcard from the images of dataset{" "}
                <a href="https://creating-new-dimensions.org/Georg-Forsters-Entdeckung-der-Suedsee/">
                  Georg-Forsters-Entdeckung-der-Suedsee
                </a>
                , by composing chromatic layers.
              </p>
              <img src="./icono.png"></img>
              <p className="wip">POST your postcard to </p>
              <p>
                GO to{" "}
                <a href="https://thesecoverthesea.solsarratea.world/">
                  {" "}
                  ↓ the sea
                </a>
              </p>
              <p className="wip">AND print it !</p>
            </section>
          </>
        ) : null}
        ...
        {editMask ? (
          <section id="mask" className="scroll">
            <section className="rgb-layout">
              <p>↓ selection: </p>
              <RgbaColorPicker
                color={shaderInfo.color}
                onChange={(value: RGBAColor) => setColor(value)}
              />
            </section>
            <section className="slider">
              <p>↓ threshold: </p>
              <Slider
                value={`${shaderInfo.deltaColor * 100}`}
                callback={(value: number) => setDeltaColor(value)}
              />
            </section>
            <section className="slider">
              <p>↓ smootheness: </p>
              <Slider
                value={`${shaderInfo.deltaSmooth * 100}`}
                callback={(value: number) => setDeltaSmooth(value)}
              />
            </section>
          </section>
        ) : null}
        ...
        {editLayer ? (
          <section id="layerc" className="scroll">
            ...
            <section className="rgb-layout">
              ...
              <RgbaColorPicker
                color={shaderInfo.color}
                onChange={(value: RGBAColor) => {
                  if (id !== null) {
                    setNewColor(id, value);
                  }
                }}
              />
            </section>
            <section className="slider">
              ...
              <Slider
                value={currentElement ? `${currentElement.mix * 100}` : "80"}
                callback={(value: number) => setMix(value)}
              />
            </section>
            <hr />
            <section className="generator">
              ...
              <button
                className="button"
                onClick={() => {
                  addLayer({ ...shaderInfo, path, pivotId: 0 });
                }}
              >
                +
              </button>
              <button
                className={`button ${
                  currentElement !== null && currentElement.pivotControls
                    ? "selected"
                    : ""
                }`}
                onClick={() => {
            ...
            <section className="world-view">
              ...
              <button
                className={`button ${state.orbitControls ? "" : "selected"}`}
                onClick={() =>
                  state.setBoolean("orbitControls", !state.orbitControls)
                }
              >
                2D
              </button>
              ...
              <button
                className="button"
                onClick={() => state.setBoolean("resetCam", true)}
              >
                R
              </button>
            </section>
            ...
            <section>
              <p className="padding-left">
                ↓ <b>show</b> | <b>hide</b> all controls on layers
              </p>
              <button
                className="button ascii"
                onClick={() => state.allPivotControls(true)}
              >
                {"(･ω･)"}
              </button>
              <button
                className="button ascii"
                onClick={() => state.allPivotControls(false)}
              >
                {"┴┤ω･)"}
              </button>
            </section>
        ...
        {data ? (
          <>
            ...
            <div className="selector">
              <button
                className=""
                onClick={() => state.setBoolean("downloadJSON", true)}
              >
                json
              </button>
              <hr />
              <button
                className=""
                onClick={() => state.setBoolean("takeScreenshot", true)}
              >
                image
              </button>
              <hr />
